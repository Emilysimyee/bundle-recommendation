  ...
  catalogList.innerHTML = `
    <div class="overflow-auto flex flex-col gap-4">
      ${allReports.map(report => `
        <div class="flex-auto items-center gap-2 p-4 border-2 border-blue-200 rounded-lg hover:border-blue-400 transition-colors bg-white">
          <input 
            type="checkbox" 
            id="report-${report.id}"
...
window.saveBundle = async function(bundleIndex) {
  ...
  const name = `Bundle ${bundleIndex + 1}`;
